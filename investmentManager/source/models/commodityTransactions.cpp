...
#include "include/models/commodityTransactions.h"
...
namespace models
{
  ...
  CComodityTransactions::CComodityTransactions(Wt::Dbo::Session &sqlSession)
  ...
  void CComodityTransactions::changeCommodity(std::string const &guid)
  {
    if (guid.empty())
    {
      clear();
  ...
  void CComodityTransactions::clear()
  {
    commodityGUID.clear();
    cacheStartIndex = 0;
    recordCache.clear();
  ...
  std::any CComodityTransactions::data(Wt::WModelIndex const &index, Wt::ItemDataRole role) const
  {
  ...
  void CComodityTransactions::readRecords(std::uint64_t rowNeeded) const
  {
  ...
  std::any CComodityTransactions::headerData(int section, Wt::Orientation orientation, Wt::ItemDataRole role) const
  {
    std::any returnValue;
  ...
  int CComodityTransactions::rowCount(Wt::WModelIndex const &parent) const
  {
    return recordCount;
  }
